...
class VggnetWsvaeMultiHead(pytorch_lightning.LightningModule):
    ...
    def __init__(self, base_model, learning_rate=1e-5):
        ...
        self.pool2 = torch.nn.MaxPool2d(4)
        self.pool3 = torch.nn.MaxPool2d(2)
        self.enc_head0 = VggnetEncHead(7, 64, 2 * self.n_latent)
        self.cls_head0 = torch.nn.Sequential(
            torch.nn.Linear(1, 4096),
            torch.nn.Sigmoid(),
            torch.nn.Linear(4096, 4096),
            torch.nn.Sigmoid(),
            torch.nn.Linear(4096, self.levels),
            torch.nn.Sigmoid(),
        )
        self.enc_head1 = VggnetEncHead(7, 128, 2 * self.n_latent)
        self.cls_head1 = torch.nn.Sequential(
            torch.nn.Linear(1, 4096),
            torch.nn.Sigmoid(),
            torch.nn.Linear(4096, 4096),
            torch.nn.Sigmoid(),
        ...
        self.enc_head2 = VggnetEncHead(7, 256, 2 * self.n_latent)
        self.cls_head2 = torch.nn.Sequential(
            torch.nn.Linear(1, 4096),
            torch.nn.Sigmoid(),
            torch.nn.Linear(4096, 4096),
            torch.nn.Sigmoid(),
        ...
        self.enc_head3 = VggnetEncHead(7, 512, 2 * self.n_latent)
        self.cls_head3 = torch.nn.Sequential(
            torch.nn.Linear(1, 4096),
            torch.nn.Sigmoid(),
            torch.nn.Linear(4096, 4096),
            torch.nn.Sigmoid(),
        ...
        for param in self.enc_head4.parameters():
            param.requires_grad = False
        #self.enc_head4.freeze()
        self.dec_head0 = VggnetDecHead(self.n_latent, 7, 64)
        self.dec_head1 = VggnetDecHead(self.n_latent, 7, 128)
        self.dec_head2 = VggnetDecHead(self.n_latent, 7, 256)
        self.dec_head3 = VggnetDecHead(self.n_latent, 7, 512)
        self.dec_head4 = m.decoder.head
        for param in self.dec_head4.parameters():
            param.requires_grad = False
        #self.dec_head4.freeze()
    ...
    def forward(self, x):
        mu, logvar, y = self.forward_enc(x)
        z = []
        for m, lvar, y in zip(mu, logvar, y):
            stdev = torch.exp(lvar / 2)
            eps = torch.randn_like(stdev)
            z.append(m + stdev * eps)
        return self.forward_dec(*z), mu, logvar, y
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        loss = torch.Tensor([0])
        loss = loss.to(self.device)
        for i,j,k,l,m in zip(x_hat, mu, logvar, y_hat, head_weights):
            kl_loss = 0.5 * torch.sum(j.pow(2) + k.exp() - k - 1) / self.n_latent
            mse_loss = torch.nn.functional.mse_loss(i, x, reduction='mean')
            ce_loss = torch.nn.functional.binary_cross_entropy(l, torch.nn.functional.one_hot(y, self.levels).float(), reduction='mean')
            pstn_loss = torch.nn.functional.mse_loss(j[:, 0], (y - self.levels / 2) / self.levels, reduction='mean')
            loss += l * (mse_loss + self.beta * kl_loss + self.alpha * ce_loss + self.gamma * pstn_loss)
        self.log('train_loss', loss)
        return loss
